...
A obtenção de um croqui DEVE (MUST) seguir a hierarquia de quatro etapas: (1) Sessão ativa em memória RAM, (2) Armazenamento permanente em `/downloads`, (3) Cache volátil em `/temp_cache` indexado pelo hash atual, e (4) Download remoto da CDN com salvamento no cache volátil. As leituras locais nas etapas 1, 2 e 3 NÃO DEVEM calcular o hash SHA-256 dos bytes em tempo de execução para preservar CPU e bateria.
