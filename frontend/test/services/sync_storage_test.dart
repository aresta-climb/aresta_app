...
void main() {
  ...
  group('SyncStorage - Indice', () {
    test('readLocalIndice retorna null se arquivo nao existe', () async {
      final result = await storage.readLocalIndice(
        '${tempDir.path}/inexistente.binarypb',
      );
      expect(result, isNull);
    });

    test(
      'readLocalIndice retorna Indice quando arquivo existe e eh valido',
      () async {
    ...
    test(
      'readLocalIndice retorna null ao tentar ler protobuf invalido (breaking change)',
      () async {
    ...
    test(
      'writeLocalIndice cria o diretorio pai caso nao exista e escreve o arquivo',
      () async {
  ...
  group('SyncStorage - readLocalCroqui', () {
    test('readLocalCroqui retorna null se nao existe', () async {
      final result = await storage.readLocalCroqui(
        '${tempDir.path}/nao_existe.binarypb',
      );
      expect(result, isNull);
    });
    ...
    test('readLocalCroqui retorna Croqui se arquivo for valido', () async {
      ...
      final croqui = Croqui()
        ..arquivosExternos.add(ArquivoExterno()..caminho = 'test');
    ...
    test(
      'readLocalCroqui retorna null ao tentar ler protobuf invalido (breaking change)',
      () async {
  ...
  group('SyncStorage - Operacoes de Arquivo', () {
    test(
      'saveTmpFile salva bytes e cria diretorio pai se necessario',
      () async {
  ...
  group('SyncStorage - validateExistingTmpFile', () {
    test('validateExistingTmpFile retorna false se nao existir', () async {
      final result = await storage.validateExistingTmpFile(
        '${tempDir.path}/nao_existe.tmp',
        'hash',
      );
      expect(result, isFalse);
    });
    ...
    test(
      'validateExistingTmpFile deleta e retorna false se expectedHash for vazio',
      () async {
    ...
    test(
      'validateExistingTmpFile deleta e retorna false se hash nao bater',
      () async {
  ...
  group('SyncStorage - applyAtomicFileUpdates', () {
    test('renomeia arquivos e deleta os que devem ser deletados', () async {
      ...
      final tmpFile2 =
          '${tempDir.path}/file2.tmp'; // Arquivo sem .txt original anterior
