...
void main() {
  group('MapaReferenciaExtension Tests', () {
    test('nome returns escalada if not empty', () {
      final ref = Mapa_Referencia(escalada: 'Via 1', setor: 'Setor 1', grupo: 'Grupo 1');
  ...
  group('DatasetResolver', () {
    ...
    setUp(() {
      ...
      setor2.escaladas.add(Escalada(viaMultiplasEnfiadas: ViaMultiplasEnfiadas(nome: 'Via 3')));
      ...
      setorGrupo.escaladas.add(Escalada(viaEsportiva: ViaEsportiva(nome: 'Via 4')));
      grupo1.setores.add(ArquivoSetor(conteudo: setorGrupo));

      pico.setoresOuGrupos.add(SetorOuGrupo(grupo: ArquivoGrupo(conteudo: grupo1)));
    });
    ...
    test('resolves setor with escalada', () {
      final res = DatasetResolver.resolve(pico: pico, setorNome: 'Setor A', escaladaNome: 'Via 1');
      expect(res.setor?.nome, 'Setor A');
      expect(res.escalada?.viaEsportiva.nome, 'Via 1');
    });

    test('resolves different types of escalada', () {
      var res = DatasetResolver.resolve(pico: pico, setorNome: 'Setor A', escaladaNome: 'Via 2');
      expect(res.escalada?.viaMovel.nome, 'Via 2');

      res = DatasetResolver.resolve(pico: pico, setorNome: 'Setor A', escaladaNome: 'Boulder 1');
      expect(res.escalada?.boulder.nome, 'Boulder 1');

      res = DatasetResolver.resolve(pico: pico, setorNome: 'Setor B', escaladaNome: 'Via 3');
      expect(res.escalada?.viaMultiplasEnfiadas.nome, 'Via 3');

      res = DatasetResolver.resolve(pico: pico, setorNome: 'Setor B', escaladaNome: 'Highline 1');
    ...
    test('throws when escalada not found in setor', () {
      expect(
        () => DatasetResolver.resolve(pico: pico, setorNome: 'Setor A', escaladaNome: 'Via Inexistente'),
        throwsException,
      );
    });

    test('resolves grupo and its setor', () {
      final res = DatasetResolver.resolve(pico: pico, grupoNome: 'Grupo 1', setorNome: 'Setor Grupo');
      expect(res.grupo?.nome, 'Grupo 1');
      expect(res.setor?.nome, 'Setor Grupo');
    });

    test('resolves grupo, setor and escalada', () {
      final res = DatasetResolver.resolve(pico: pico, grupoNome: 'Grupo 1', setorNome: 'Setor Grupo', escaladaNome: 'Via 4');
      expect(res.grupo?.nome, 'Grupo 1');
      expect(res.setor?.nome, 'Setor Grupo');
    ...
    test('resolves setor globally if not inside explicitly given root (fallback)', () {
      // Setor Grupo is inside Grupo 1.
      final res = DatasetResolver.resolve(pico: pico, setorNome: 'Setor Grupo');
      expect(res.setor?.nome, 'Setor Grupo');
    });
    ...
    group('resolveReferencia', () {
      test('resolves escalada using defaultSetorNome', () {
        ...
        final res = DatasetResolver.resolveReferencia(pico: pico, referencia: ref, defaultSetorNome: 'Setor A');
        expect(res.setor?.nome, 'Setor A');
        expect(res.escalada?.viaEsportiva.nome, 'Via 1');
      });

      test('resolves escalada and uses defaultGrupoNome and defaultSetorNome', () {
        final ref = Mapa_Referencia(escalada: 'Via 4');
        final res = DatasetResolver.resolveReferencia(pico: pico, referencia: ref, defaultGrupoNome: 'Grupo 1', defaultSetorNome: 'Setor Grupo');
        expect(res.grupo?.nome, 'Grupo 1');
        expect(res.setor?.nome, 'Setor Grupo');
        expect(res.escalada?.viaEsportiva.nome, 'Via 4');
      });

      test('resolves explicit group, explicit setor, explicit escalada from ref', () {
        final ref = Mapa_Referencia(grupo: 'Grupo 1', setor: 'Setor Grupo', escalada: 'Via 4');
        final res = DatasetResolver.resolveReferencia(pico: pico, referencia: ref, defaultGrupoNome: 'Grupo Errado', defaultSetorNome: 'Setor Errado');
        expect(res.grupo?.nome, 'Grupo 1');
        expect(res.setor?.nome, 'Setor Grupo');
        expect(res.escalada?.viaEsportiva.nome, 'Via 4');
      });
      ...
      test('throws when referenced entity does not exist', () {
        final ref = Mapa_Referencia(escalada: 'Via 1');
        expect(
          () => DatasetResolver.resolveReferencia(pico: pico, referencia: ref, defaultSetorNome: 'Setor Inexistente'),
          throwsException,
        );
      });
    });
