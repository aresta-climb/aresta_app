...
void main() {
  ...
  testWidgets('HomePage pull-to-refresh quando não há croquis baixados mostra snackbar adequado', (tester) async {
    ...
    expect(find.text('Nenhum croqui baixado para atualizar.'), findsOneWidget);
    verifyNever(() => mockSync.syncIndex(auto: false));
  });

  testWidgets('HomePage pull-to-refresh aciona sincronização se houver croquis baixados', (tester) async {
    final mockRepo = MockDatasetRepository();
    final mockSync = MockSyncService();
    final ValueNotifier<TopoDataset?> activeDataset = ValueNotifier(
      TopoDataset(availablePicos: [], downloadedPicos: [{'id': '1', 'nome': 'Pico 1'}]),
    );
