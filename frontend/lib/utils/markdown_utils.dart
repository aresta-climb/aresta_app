class MarkdownUtils {
  ...
  static String cleanModalContent(String rawContent, String modalTitle) {
    ...
    final frontmatterRegex = RegExp(r'^---+\s*\r?\n(?:[\s\S]*?\r?\n)?---+\s*\r?\n?');
    content = content.replaceFirst(frontmatterRegex, '').trimLeft();

    final lines = content.split('\n');
    if (lines.isNotEmpty) {
      final firstLine = lines.first.trim();
      final cleanFirstLine = firstLine.replaceAll(RegExp(r'^#+\s*'), '').trim().toLowerCase();
      ...
      if (cleanFirstLine.isNotEmpty && cleanFirstLine.length < 100 && 
          (cleanFirstLine.contains(cleanTitle) || cleanTitle.contains(cleanFirstLine))) {
