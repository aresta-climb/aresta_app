...
class MapHierarchyResolver {
  ...
  static MapDestination? resolveUpDestination({
    ...
    if (setorContext != null &&
        grupoContext != null &&
        grupoContext.mapas.isNotEmpty) {
      // Retorna todos os mapas do Grupo
      final mapasData = grupoContext.mapas
          .map(
            (m) => CarrosselItemData(
      ...
      return MapDestination(
        ...
        setorContext:
            null, // Subiu de Setor para Grupo, então perde o contexto de Setor
      );
    }
    ...
    if (pico.hasMapasGerais() &&
      ...
      final mapasData = pico.mapasGerais.conteudo.mapas
          .map((m) => CarrosselItemData(mapaCaminhoImagem: m.caminhoImagemMapa))
          .toList();
