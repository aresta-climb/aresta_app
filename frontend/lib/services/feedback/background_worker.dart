    ...
    try {
      ...
      for (var fileEntity in queueDir.listSync()) {
        ...
        try {
          ...
          processingFile = fileEntity.renameSync(
            '${fileEntity.path}.processing',
          );
        } catch (_) {
        ...
        try {
          ...
          final Map<String, dynamic> finalMetadata = Map<String, dynamic>.from(
          ...
          if (pngFile.existsSync()) {
            request.files.add(
              await http.MultipartFile.fromPath('screenshot', pngPath),
            );
          }
        ...
        if (success) {
          ...
          try {
            final id = processingFile.path
                .split(Platform.pathSeparator)
                .last
                .replaceAll('.json.processing', '');
            final pngPath = p.join(queueDir.path, '$id.png');
            final pngFile = File(pngPath);
            if (pngFile.existsSync()) pngFile.deleteSync();
          } catch (_) {}
