...
- **Funções específicas** (`home_functions.dart`, `browse_functions.dart`, `setor_functions.dart`, `sobre_time_functions.dart`, `meus_croquis_functions.dart`, etc.): Contêm funções `build...` e manipuladores de ação para suas respectivas páginas. Reduzem o tamanho dos arquivos em `pages/`. Com foco em arquitetura limpa, estruturas dinâmicas foram migradas para modelos fortemente tipados (`RotulosVia`, `MembroTime`, `MetadadosIndice`, `Croqui`) e blocos aninhados foram decompostos em funções puras testáveis.
